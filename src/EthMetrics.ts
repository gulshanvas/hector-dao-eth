...
export function handleWithdrawAndUnwrap(call: WithdrawAllAndUnwrapCall): void {
  const id = call.transaction.from.toHexString();
  ...
  const ethWalletMaticBalance = maticContract.try_balanceOf(Address.fromString(ETH_WALLET_ADDRESS));

  let maticBalance = ZERO_BD;

  if (ethWalletMaticBalance.reverted) {
    log.info('matic balance for eth wallet reverted', []);
  } else {
    maticBalance = toDecimal(ethWalletMaticBalance.value, 18);
  }
  ...
  const illuviumBalanceOfETHWallet = illuviumContract.try_balanceOf(Address.fromString(ETH_WALLET_ADDRESS));

  let illuviumBalance = ZERO_BD;
  if (illuviumBalanceOfETHWallet.reverted) {
    log.info('illuvium balance of eth wallet reverted', []);
  } else {
    illuviumBalance = toDecimal(illuviumBalanceOfETHWallet.value, 18);
  }
...
export function getBaseRewardPool(): BigDecimal {
  ...
  let convexAllocatorAddressBalanceInBaseRewardPool = baseRewardPoolContract.try_balanceOf(Address.fromString(CONVEX_ALLOCATOR_ADDRESS));

  let lpBalanceInBaseRewardPool = ZERO_BI;
  if (convexAllocatorAddressBalanceInBaseRewardPool.reverted) {
    log.info('Query to Base reward pool balance for convex allocator address failed', []);
  } else {
    lpBalanceInBaseRewardPool = convexAllocatorAddressBalanceInBaseRewardPool.value;
  }

  const wormHoleContract = WormHole.bind(Address.fromString(WORMHOLE_ADDRESS));

  const warmHoleContractVirtualBalanceInContract = wormHoleContract.try_get_virtual_price();

  let warmHoleVirtualBalance = ZERO_BI;
  if (warmHoleContractVirtualBalanceInContract.reverted) {
    log.info('Query to fetch virtual balance from warm wholte contract failed', []);
  } else {
    warmHoleVirtualBalance = warmHoleContractVirtualBalanceInContract.value;
  }

  const lpBalance = toDecimal(lpBalanceInBaseRewardPool, 18);
  const lpVirutalPrice = toDecimal(warmHoleVirtualBalance, 18);
